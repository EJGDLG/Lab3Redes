...
def main():
    p = argparse.ArgumentParser(description="RouterSim node (sockets/XMPP)")
    ...
    p.add_argument("--jid", help="JID XMPP (por defecto, el de names-*.txt para el ID)")
    p.add_argument("--password", help="Password XMPP")
    p.add_argument("--xmpp-host", help="Host/IP del servidor XMPP (opcional)")
    p.add_argument("--xmpp-port", type=int, default=16379, help="Puerto XMPP")
    p.add_argument("--xmpp-tls", action="store_true", help="Forzar TLS")
    ...
    node = Node(
        ...
        xmpp_host=args.xmpp_host, xmpp_port=args.xmpp_port, xmpp_tls=args.xmpp_tls
    )
    ...
    if args.send:
        if not args.message:
            print("Falta el mensaje a enviar")
            sys.exit(1)
        time.sleep(3.0)
        node.send_user_message(args.send, args.message)
        time.sleep(2.0)
        return
